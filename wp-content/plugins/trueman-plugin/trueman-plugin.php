<?php
/**
 * Plugin Name: Trueman Plugin
 * Plugin URI: https://bslthemes.com/
 * Description: This plugin it's designed for Trueman Theme
 * Version: 1.2.1
 * Author: bslthemes
 * Author URI: https://bslthemes.com/
 * Text Domain: trueman-plugin
...
if ( ! defined( 'TRUEMAN_PLUGIN_PATH' ) ) {
	define( 'TRUEMAN_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
}
if ( ! defined( 'TRUEMAN_PLUGIN_URI' ) ) {
	define( 'TRUEMAN_PLUGIN_URI', plugin_dir_url( __FILE__ ) );
}

// Main Class
if ( ! class_exists( 'TruemanPlugin' ) ) {
	...
	class TruemanPlugin {
		...
		public function plugin_load_textdomain() {
			...
			load_plugin_textdomain( 'trueman-plugin', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
		...
		public function init_hooks() {
			...
			/* fixed theme update */
			function trueman_fix_trans_after_update() {
			...
			add_action( 'admin_init', 'trueman_fix_trans_after_update' );
		...
		public function init_cpt() {
			...
			require_once TRUEMAN_PLUGIN_PATH . 'inc/custom-post-types.php';
		}

		public function init_acf_ext() {
			/* include acf fields extention */
			require_once TRUEMAN_PLUGIN_PATH . 'acf-ext/acf-ui-google-font/acf-ui-google-font.php';
			require_once TRUEMAN_PLUGIN_PATH . 'acf-ext/acf-cf7/acf-cf7.php';
			require_once TRUEMAN_PLUGIN_PATH . 'acf-ext/acf-fa/acf-font-awesome-font.php';
		}

		public function init_theme_helpers() {
			/* include social share */
			require_once TRUEMAN_PLUGIN_PATH . 'inc/social-share/social-share.php';
		}

		public function init_elementor_addons() {
			/* include elementor addons */
			require_once TRUEMAN_PLUGIN_PATH . 'elementor/functions.php';
		}

		public function init_dashboard() {
			/* include theme dashboard */
			require TRUEMAN_PLUGIN_PATH . 'admin/dashboard-theme-helper.php';
			require TRUEMAN_PLUGIN_PATH . 'admin/dashboard-theme-init.php';
			require TRUEMAN_PLUGIN_PATH . 'admin/dashboard-theme-activation.php';
		}

		public function dismiss() {
			if ( isset( $_GET['trueman-dismiss'] ) && check_admin_referer( 'trueman-dismiss-' . get_current_user_id() ) ) {
				update_user_meta( get_current_user_id(), 'trueman_dismissed_notice', 1 );
			}
		}

		public function after_theme_upgrade( $upgrader_object, $options ) {
			if ( ( $options['action'] == 'install' ) && $options['type'] == 'theme' ) {
				 if ( $upgrader_object->new_theme_data['Name'] == 'Trueman' ) {
					 self::update_dismiss();
				 }
			}
			if ( ( $options['action'] == 'install' ) && $options['type'] == 'plugin' ) {
				 if ( $upgrader_object->new_plugin_data['Name'] == 'Trueman Plugin' ) {
					 self::update_dismiss();
				 }
			}
		}
...
$truemanPlugin = new TruemanPlugin();
$truemanPlugin->init();

register_activation_hook( __FILE__, array( $truemanPlugin, 'activation' ) );
register_deactivation_hook( __FILE__, array( $truemanPlugin, 'deactivation' ) );
